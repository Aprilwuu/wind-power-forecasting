...
# -----------------------------
# NEW: export plot-friendly npz
# -----------------------------
def _pick_center_column(df: pd.DataFrame) -> Optional[str]:
    """
    Prefer true model center if available.
    For MC dropout, likely candidates are q50 / mean / y_pred / pred.
    """
    for c in ["q50", "mean", "y_pred", "pred", "mu"]:
        if c in df.columns:
            return c
    return None


def _csv_to_plot_npz(
    csv_path: Path,
    npz_path: Path,
    *,
    y_col: str = "y_true",
    lo_col: str = "q05",
    hi_col: str = "q95",
    lo_cal_col: Optional[str] = None,
    hi_cal_col: Optional[str] = None,
) -> Optional[Path]:
    """
    Convert a csv prediction file to a plot-friendly npz.

    Raw npz fields:
      y_true, q05, q95, [q50/mean/pred if available]

    Cal npz fields:
      y_true, q05, q95, q05_cal, q95_cal, [q50/mean/pred if available]
    """
    if not csv_path.exists():
        return None

    df = pd.read_csv(csv_path)
    if y_col not in df.columns or lo_col not in df.columns or hi_col not in df.columns:
        logger.warning(f"Skip export from {csv_path}: missing one of required columns {y_col}, {lo_col}, {hi_col}")
        return None

    arrays: Dict[str, np.ndarray] = {
        "y_true": df[y_col].to_numpy(dtype=np.float32).reshape(-1, 1),
        "q05": df[lo_col].to_numpy(dtype=np.float32).reshape(-1, 1),
        "q95": df[hi_col].to_numpy(dtype=np.float32).reshape(-1, 1),
    }

    center_col = _pick_center_column(df)
    if center_col is not None:
        arrays[center_col] = df[center_col].to_numpy(dtype=np.float32).reshape(-1, 1)

    if lo_cal_col and hi_cal_col and lo_cal_col in df.columns and hi_cal_col in df.columns:
        arrays["q05_cal"] = df[lo_cal_col].to_numpy(dtype=np.float32).reshape(-1, 1)
        arrays["q95_cal"] = df[hi_cal_col].to_numpy(dtype=np.float32).reshape(-1, 1)
...
def _export_plot_npz_from_out_dir(
    *,
    out_dir: str,
    split_type: str,
    target_picp: float,
    ...
    """
    Export raw/cal plot-friendly npz from pipeline outputs.

    Returns paths:
      raw_val_npz, raw_test_npz, cal_val_npz, cal_test_npz
    """
    out = {
        "raw_val_npz": "",
        "raw_test_npz": "",
    ...
    if split_type == "temporal":
        raw_val_csv = d / "preds_val_mc.csv"
        raw_test_csv = d / "preds_test_mc.csv"
    else:
        raw_val_csv = d / "preds_inner_val_mc.csv"
        raw_test_csv = d / "preds_outer_test_mc.csv"

    # raw npz
    raw_val_npz = raw_val_csv.with_suffix(".npz")
    raw_test_npz = raw_test_csv.with_suffix(".npz")

    p1 = _csv_to_plot_npz(raw_val_csv, raw_val_npz, y_col="y_true", lo_col="q05", hi_col="q95")
    p2 = _csv_to_plot_npz(raw_test_csv, raw_test_npz, y_col="y_true", lo_col="q05", hi_col="q95")

    if p1:
        out["raw_val_npz"] = str(p1)
    if p2:
        out["raw_test_npz"] = str(p2)

    # cal csvs from conformal_widen_exp_dir
    raw_val_cal_csv = raw_val_csv.with_name(raw_val_csv.stem + "_cal" + raw_val_csv.suffix)
    raw_test_cal_csv = raw_test_csv.with_name(raw_test_csv.stem + "_cal" + raw_test_csv.suffix)

    if raw_val_cal_csv.exists():
        cal_val_npz = raw_val_cal_csv.with_suffix(".npz")
        p3 = _csv_to_plot_npz(
            raw_val_cal_csv,
            cal_val_npz,
            y_col="y_true",
        ...
        if p3:
            out["cal_val_npz"] = str(p3)

    if raw_test_cal_csv.exists():
        cal_test_npz = raw_test_cal_csv.with_suffix(".npz")
        p4 = _csv_to_plot_npz(
            raw_test_cal_csv,
            cal_test_npz,
            y_col="y_true",
...
def main() -> None:
    ...
    target_picp = float(prob_cfg.get("target_picp", 0.9))
    ...
    for cand_name, cand_override in candidates.items():
        ...
        for seed in args.seeds:
            if split_type == "temporal":
                ...
                exported = _export_plot_npz_from_out_dir(
                    out_dir=result.out_dir,
                ...
                row = {
                    ...
                    "plot_raw_val_npz": exported["raw_val_npz"],
                    "plot_raw_test_npz": exported["raw_test_npz"],
                    "plot_cal_val_npz": exported["cal_val_npz"],
                    "plot_cal_test_npz": exported["cal_test_npz"],
                    **conf,
                }
                runs.append(row)
