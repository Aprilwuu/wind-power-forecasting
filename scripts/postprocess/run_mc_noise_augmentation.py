...
"""
Add residual noise to MC-dropout prediction intervals as a post-hoc correction.

This script:
    1. Estimates residual noise scale (sigma_resid) from inner validation predictions
    2. Combines MC-dropout uncertainty with residual noise
    3. Produces noise-augmented prediction intervals for the outer test set

Typical usage:
    python -m scripts.postprocess.run_add_noise \
        --inner path/to/preds_inner_val_mc.npz \
        --outer path/to/preds_outer_test_mc.npz

Note:
    This script is specific to the TCN-MC dropout + residual noise augmentation pipeline.
"""
